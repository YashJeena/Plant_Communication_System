...
const int msensor = A1; // Moisture Sensor data pin Connected to analog pin A1 in 
Arduino const int Vin = A0; // Pin to receive analog readings from the photoresistor
...
const int buzz = 12;
int pos = 0;
LedControl lc = LedControl(DIN, CLK, CS, 0); // Defininng pins to control the 8X8 dot 
matrix LiquidCrystal_I2C lcd(0x27, 16, 2); // Defining LCD connections to I2C bus
DHT dht(DHTPIN, DHTTYPE); // Defining and Setting up DHT 
sensor Servo myservo;
int msvalue = 0; // Initial Value of Moisture Sensor
int light = 0; // Initial value of voltage across photoresistor
float tempC = 0.0; // Assigning variable to calculate the Temperature 
score float humC = 0.0; // Assigning variable to calculate the Humidity 
score float msenC = 0.0; // Assigning variable to calculate the Moisture 
score float lightC = 0.0; // Assigning variable to calculate the Sunlight 
score float hum; // Stores humidity value in percent
float temp; // Stores temperature value in Celcius
float score = 0.0;
byte smile[8] = {0x3C,0x42,0xA5,0x81,0xA5,0x99,0x42,0x3C}; // Byte array to store the "Smile Expression"
byte neutral[8] = {0x3C,0x42,0xA5,0x81,0xBD,0x81,0x42,0x3C}; // Byte array to store the "Neutral 
Expression"
...
void setup() {
  ...
  pinMode(msensor, INPUT); //giving analog pin address (A1) to Arduino for Moisture 
  sensor pinMode(Vin, INPUT); //giving analog pin address (A2) to Arduino for photoresistor
...
//Soil Moisture Parameters
  msvalue = analogRead(msensor); //Reading Value from Moisture Sensor //Get Soil Moisture values 
  from analog pin
...
//Sunlight Parameters
  light = analogRead(Vin); //Reading Voltage across the photoresistor to calculate the light 
  intensity Serial.print("The PhotoResistor value is:");
...
//All parameters have been checked and we now calculate the 
  score score = ((tempC + humC + msenC + lightC) / 4);
  if (score >= 2.5) {
    lcd.setCursor(0, 0);
    lcd.print("Score: ");
    lcd.print(score);
    lcd.print("/3.00");
    lcd.setCursor(0, 1);
    lcd.print(" Plant is HAPPY "); //Print how the plant feels on bottom 
    line printByte(smile); //Display Plant's expression on the DOT Matrix 
    delay(3000);
  } else if (score >= 2) {
    lcd.setCursor(0, 0); //Print Caretaking score on top 
    line lcd.print("Score: ");
    lcd.print(score);
    lcd.print("/3.00");
    lcd.setCursor(0, 1); //Print how the plant feels on bottom line 
    lcd.print(" Plant is Fine"); //Display Plant's expression on the DOT 
    Matrix myservo.write(0);
    printByte(neutral);
    delay(3000); 
  } else {
    lcd.setCursor(0, 0); //Print Caretaking score on top 
    ...
    lcd.print("/3.00");
    lcd.setCursor(0, 1); //Print how the plant feels on bottom line 
    lcd.print(" Plant is SAD"); //Display Plant's expression on the DOT 
    Matrix printByte(sad);
    tone(buzz,261);
